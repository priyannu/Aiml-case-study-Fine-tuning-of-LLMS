...
Code 1: Rectification Script

What it does:

Reads glossary_full.csv (124 terms).

Filters only the ambiguous terms (Ambiguous = Yes).

Generates a dynamic rectification/suggestion for each ambiguous term.

Saves the output in ambiguous_rectified.csv.

README coverage: ✅

Input/output CSVs mentioned.

Auto-generation of rectifications explained.

Example output included.


Code 2: Main Model + GUI

What it does:

Loads both glossary_full.csv and ambiguous_rectified.csv.

Preprocesses and balances the dataset.

Fine-tunes DistilBERT for binary classification (Ambiguous / Clear).

Computes Precision, Recall, F1-Score and optimizes threshold.

Builds an interactive Tkinter GUI to:

Enter a term + optional clarification

Get a prediction label and confidence

See dynamic bar graph of probabilities

Receive a suggestion for ambiguous terms

README coverage: ✅

Dataset usage explained.

Model fine-tuning and threshold optimization mentioned.

GUI features, metrics display, and bar graph described.
